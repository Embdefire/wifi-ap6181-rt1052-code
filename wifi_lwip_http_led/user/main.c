...
void ExperimentalDescription()
{
	...
	PRINTF("实验中使用TCP协议传输数据，电脑浏览器/手机浏览器 作为TCP Client ，开发板作为TCP Server\n\n");

	PRINTF("热点的IP地址、端口号在appliance.h文件中修改\n\n");

	PRINTF("本例程参考<<LwIP应用实战开发指南>>第20章 HTTP 服务器\n\n");

	PRINTF("打开手机的浏览器或电脑浏览器，输入开发板的IP地址(如192.168.1.200)，按下回车即可\n\n");
	
	PRINTF("若你是用的是电脑浏览器，需要使用一个路由器连接电脑，然后，路由器桥接到WIFI热点\n\n");
	
	PRINTF("若你是用的是手机浏览器，需要使用手机连接wifi 热点即可\n\n");
